                      ...
                      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        {loading ? (
                          ...
                          <span className="text-xs text-red-400 flex items-center gap-1">
                            ⚠ Error al descargar · <button onClick={(e) => { e.stopPropagation(); onRefresh(layer.id); }} className="underline">reintentar</button>
                          </span>
                        ) : info ? (
                          <>
                            <span className="text-xs text-gray-400">
                              {count > 0 ? (
                                <span className="font-medium text-gray-600">{count} elementos</span>
                              ) : "sin datos"}
                            </span>
                            <span className="text-gray-300">·</span>
                            <span className="text-xs text-gray-400">{timeAgo(info.cachedAt)}</span>
                            {!info.fresh && (
                              <span className="text-xs bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full">caché</span>
                            )}
                          </>
                        ) : (
